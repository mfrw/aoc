...
impl utils::Solver<22> for Solver {
    type Part1 = i64;
    ...
    fn part1(&self, input: &str) -> Result<Self::Part1, Box<dyn std::error::Error>> {
        ...
        for col in 0..m.map.width {
            if Some(&Tile::Space) == m.map.tiles.get(&(0, col)) {
            ...
            match step {
                ...
                Move::Forward(n) => {
                    for _ in 0..*n {
                        ...
                        if let Some(tile) = m.map.tiles.get(&next_pos) {
                            match tile {
                                Tile::Space => pos = next_pos,
                                Tile::Wall => break,
                            }
                        } else {
                            pos = m.map.wrap(&pos, facing);
                        }
                    }
                }
            }
        ...
        Ok((pos.0 + 1) * 1000 + (pos.1 + 1) * 4 + facing as i64)
    }
                ...
                Move::Forward(n) => {
                    for _ in 0..*n {
                        ...
                        if let Some(tile) = m.map.tiles.get(&next_pos) {
                            ...
                            let (new_facing, new_pos) = m.map.wrap_cube(&pos, facing);
                            facing = new_facing;
                            pos = new_pos;
                        }
                    }
        ...
        if ch[0].is_ascii_digit() {
            ...
            for c in ch {
                match c {
                    ...
                    m => {
                        monkey.steps.push(Move::Forward(n));
                        n = 0;
                        match m {
                            'R' => monkey.steps.push(Move::Right),
                            'L' => monkey.steps.push(Move::Left),
            ...
            for (col, c) in ch.iter().enumerate() {
                match c {
                    ' ' => {}
                    '#' => monkey.map.insert((row as i64, col as i64), Tile::Wall),
                    '.' => monkey.map.insert((row as i64, col as i64), Tile::Space),

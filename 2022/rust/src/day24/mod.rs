...
impl utils::Solver<24> for Solver {
    type Part1 = i32;

    type Part2 = i32;

    fn part1(&self, input: &str) -> Result<Self::Part1, Box<dyn std::error::Error>> {
        let l = parse_input(input);
        let mut valley = make_valley(l);
        let sol = valley.find_shortest_path_time(valley.start, valley.end, 0);
        Ok(sol)
    }
    ...
    fn part2(&self, input: &str) -> Result<Self::Part2, Box<dyn std::error::Error>> {
        ...
        let mut valley = make_valley(l);
        let sol = valley.find_shortest_path_time(valley.start, valley.end, 0);
        let time_back = valley.find_shortest_path_time(valley.end, valley.start, sol);
        let final_sol = valley.find_shortest_path_time(valley.start, valley.end, time_back);
        Ok(final_sol)
    }
}

fn parse_input(i: &str) -> Vec<String> {
    i.lines().map(str::to_string).collect()
}
